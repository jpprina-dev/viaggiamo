...
from app.schemas.trip import TripCreate, Trip as TripSchema, TripUpdate, TripWithDriver
...
async def read_my_trips(
    ...
    query = select(Trip).where(Trip.driver_id == current_user.id).order_by(Trip.departure_time)
    result = await db.execute(query)
    return result.scalars().all()
